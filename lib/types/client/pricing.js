 ...
 */
export function isPeakAt(date, windows) {
 ...
 */
export function nextRegimeChange(date, windows) {
 ...
 */
export function applicableRate(model, rates, windows, peakMultiplier, date) {
    const base = rates[model];
    if (base === undefined)
        return undefined;
    if (!isPeakAt(date, windows))
        return base;
    return {
 ...
 */
export function sessionCostUsd(usage, rate) {
